...
export interface Provider {
  ...
  // 新增：是否为商业合作伙伴
  isPartner?: boolean;
  // 可选：供应商元数据（仅存于 ~/.cc-switch-cometix/config.json，不写入 live 配置）
  meta?: ProviderMeta;
  // 图标配置
...
// 存储在本地 ~/.cc-switch-cometix/settings.json，不随数据库同步
export interface Settings {
  ...
  // 覆盖 Claude Code (Cometix) 独立配置目录（可选）
  claudeCometixConfigDir?: string;
  // 覆盖 Codex 配置目录（可选）
