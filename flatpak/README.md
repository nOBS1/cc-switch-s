...
This directory contains the Flatpak manifest for CC Switch Cometix (`com.ccswitch.cometix`), used to convert the generated `.deb` artifact into an installable `.flatpak` package via CI or local builds.
...
```bash
cp "$(find src-tauri/target/release/bundle -name '*.deb' | head -n 1)" flatpak/cc-switch-cometix.deb
```
...
flatpak build-bundle --runtime-repo=https://flathub.org/repo/flathub.flatpakrepo flatpak-repo CC-Switch-Cometix-Linux.flatpak com.ccswitch.cometix
```

4) Install and run:

```bash
flatpak install --user ./CC-Switch-Cometix-Linux.flatpak
flatpak run com.ccswitch.cometix
```
...
```yaml
  - --filesystem=~/.cc-switch-cometix:create
