...
fn probe_path_default_version(tool: &str) -> ShellProbe {
    ...
    let real = std::fs::canonicalize(&path_default).unwrap_or_else(|_| path_default.clone());
...
fn resolve_path_default(
    ...
    let current_path = effective_path_os().unwrap_or_default();
    let child = windows_path_lookup_command(executable, &current_path)
        .spawn()
        .map_err(|e| format!("Failed to locate {tool}: {e}"))?;
    let out = wait_child_output(child, deadline)?;
    if !out.status.success() {
...
fn locate_default_tool(
    ...
    let path_default = resolve_path_default(tool, deadline)?.filter(|path| {
        let real = std::fs::canonicalize(path).unwrap_or_else(|_| path.clone());
        tool_installation_matches(tool, path, &real)
    });
    ...
    for dir in build_tool_search_paths(tool) {
        for candidate in tool_executable_candidates(tool, &dir) {
            ...
            if !tool_installation_matches(tool, &candidate, &real) {
                continue;
            }
