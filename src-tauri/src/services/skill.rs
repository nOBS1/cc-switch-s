...
impl SkillService {
    ...
    fn remove_skill_from_app(skill: &InstalledSkill, app: &AppType) -> Result<()> {
        if !Self::scope_owns_app(skill, app) {
            return Err(anyhow!(
                "Skill {} belongs to a different Claude domain",
                skill.id
        ...
        if !matches!(app, AppType::Pi) {
            return Self::remove_from_app(&skill.directory, app);
        }
        ...
        let app_dir = Self::get_distinct_app_skills_dir(source_root, app)?;
    ...
    pub async fn check_updates(&self, db: &Arc<Database>) -> Result<Vec<SkillUpdateInfo>> {
        ...
        for ((owner, name, branch), group_skills) in &repo_groups {
            ...
            for skill in group_skills {
                ...
                let skill_ssot_dir = Self::get_skill_ssot_dir(skill)?;
    ...
    pub fn migrate_storage(
        ...
        for skill in &skills {
            ...
            let pi_deployment = if pi_uses_source_root {
                None
            } else {
        ...
        for (scoped_relative, directory, deployment) in pi_deployments {
            let source_root = scoped_relative
                .as_ref()
                .map(|relative| new_dir.join(relative))
                .unwrap_or_else(|| new_dir.clone());
        ...
        for (scoped_relative, directory) in pi_native_sources {
            let source_root = scoped_relative
                .as_ref()
                .map(|relative| new_dir.join(relative))
                .unwrap_or_else(|| new_dir.clone());
            let source = source_root.join(&directory);
            if let Err(err) = Self::sync_source_to_app(&source, &directory, &AppType::Pi) {
                result.errors.push(format!("{directory}: {err}"));
            }
        }
    ...
    pub fn toggle_app(db: &Arc<Database>, id: &str, app: &AppType, enabled: bool) -> Result<()> {
        ...
        if enabled {
            ...
            Self::remove_skill_from_app(&skill, app)?;
        }
    ...
    pub fn import_from_apps(
        ...
        for selection in imports {
            ...
            for (scope, mut apps) in Self::split_import_apps(allowed_apps) {
                ...
                if scope.is_none() {
                    // Pi has no persisted enabled flag: its native directory is
    ...
    fn persist_and_sync_new_skill(
        ...
        let source = Self::get_skill_ssot_dir(skill)?.join(&skill.directory);
        Self::preflight_install_destination(&source, &skill.directory, app)?;
        db.save_skill(skill)?;
        if let Err(error) = Self::sync_skill_to_app(skill, app) {
