...
mod tests {
    ...
    use crate::config::{
        get_claude_cometix_settings_path, get_claude_settings_path, read_json_file, write_json_file,
    };
    ...
    #[tokio::test]
    #[serial]
    async fn update_current_cometix_provider_never_writes_official_claude() {
        let _home = TempHome::new();
        crate::settings::reload_settings().expect("reload settings");

        let official_path = get_claude_settings_path();
        fs::create_dir_all(official_path.parent().expect("official config dir"))
            .expect("create official config dir");
        let official_sentinel = br#"{"official":"must-stay-byte-identical"}"#;
        fs::write(&official_path, official_sentinel).expect("seed official settings");
        ...
        let cometix_live: Value = read_json_file(&get_claude_cometix_settings_path())
            .expect("read Cometix live settings");
        assert_eq!(
            cometix_live["env"]["ANTHROPIC_BASE_URL"].as_str(),
            Some("https://api.new.example")
        );
        assert_eq!(
            fs::read(&official_path).expect("read official settings"),
            official_sentinel,
            "editing Cometix must not touch official Claude settings"
        );
    }
