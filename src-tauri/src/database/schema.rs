...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    16 => {
                        log::info!(
                            "迁移数据库从 v16 到 v17（添加独立 Cometix 域与会话用量去重账本）"
                        );
    ...
    fn ensure_cometix_domain_columns(conn: &Connection) -> Result<(), AppError> {
        if Self::table_exists(conn, "mcp_servers")? {
            Self::add_column_if_missing(
                conn,
                "mcp_servers",
    ...
    /// v16 -> v17: add the fork's independent Cometix flags while preserving
    /// upstream's durable request identity ledger.
...
mod tests {
    ...
    #[test]
    fn migrate_v16_to_v17_preserves_both_cometix_flags_and_dedup_ledger() -> Result<(), AppError> {
        let conn = Connection::open_in_memory()?;
        conn.execute_batch(
            "CREATE TABLE mcp_servers (
                id TEXT PRIMARY KEY,
        ...
        let mcp_flags: (i64, i64) = conn.query_row(
            "SELECT enabled_claude, enabled_claude_cometix FROM mcp_servers WHERE id = 'mcp-1'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )?;
        let skill_flags: (i64, i64) = conn.query_row(
            "SELECT enabled_claude, enabled_claude_cometix FROM skills WHERE id = 'skill-1'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )?;
