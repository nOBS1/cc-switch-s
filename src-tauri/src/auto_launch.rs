...
/// 将 `/path/to/CC Switch Cometix.app/Contents/MacOS/cc-switch-cometix`
/// 转换为 `/path/to/CC Switch Cometix.app`
#[cfg(target_os = "macos")]
...
fn get_auto_launch() -> Result<AutoLaunch, AppError> {
    // Keep the OS login-item/registry key independent from upstream CC Switch.
    let app_name = "CC Switch Cometix";
    let exe_path =
...
mod tests {
    ...
    fn test_get_macos_app_bundle_path_valid() {
        let exe_path = std::path::Path::new(
            "/Applications/CC Switch Cometix.app/Contents/MacOS/cc-switch-cometix",
        );
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from(
                "/Applications/CC Switch Cometix.app"
            ))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_with_spaces() {
        let exe_path = std::path::Path::new(
            "/Users/test/My Apps/CC Switch Cometix.app/Contents/MacOS/cc-switch-cometix",
        );
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from(
                "/Users/test/My Apps/CC Switch Cometix.app"
            ))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_not_in_bundle() {
        let exe_path = std::path::Path::new("/usr/local/bin/cc-switch-cometix");
        let result = get_macos_app_bundle_path(exe_path);
    ...
    fn test_get_macos_app_bundle_path_dev_build() {
        ...
        let exe_path = std::path::Path::new("/Users/dev/project/target/debug/cc-switch-cometix");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(result, None);
    }
}
